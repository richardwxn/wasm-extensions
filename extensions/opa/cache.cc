...
uint64_t computeHash(const O &payload) {
  const uint64_t kMul = static_cast<uint64_t>(0x9ddfea08eb382d69);
  uint64_t h = 0;
  h += std::hash<std::string>()(payload.source_principal) * kMul;
  h += std::hash<std::string>()(payload.destination_service) * kMul;
  h += std::hash<std::string>()(payload.request_operation) * kMul;
...
bool ResultCache::check(const OPAPayload &param,
  ...
  if (entry.second + valid_duration_nano_ > timestamp) {
    use(hash);
    allowed = entry.first;
    return true;
  }
